...
class TenderResourceTest(BaseCompetitiveDialogEUWebTest):
    ...
    def test_stage2_EU(self):
        ...
        with open(TARGET_DIR + 'stage2/EU/update-tender-after-enqiery-with-update-periods.http', 'w') as self.app.file_obj:
            tenderPeriod_endDate = get_now() + timedelta(days=8)
            response = self.app.patch_json('/tenders/{}?acc_token={}'.format(tender['id'], owner_token),
                                           {'data': {
...
class TenderResourceTestStage2UA(BaseCompetitiveDialogUAStage2WebTest):
    ...
    def setUp(self):
        ...
        if self.docservice:
            self.setUpDS()
            self.app.app.registry.docservice_url = 'http://{}'.format(DOCS_HOST)
    ...
    def test_stage2_UA(self):
        ...
        with open(TARGET_DIR + 'stage2/UA/update-tender-after-enqiery-with-update-periods.http', 'w') as self.app.file_obj:
            tenderPeriod_endDate = get_now() + timedelta(days=8)
            response = self.app.patch_json('/tenders/{}?acc_token={}'.format(tender['id'], owner_token),
                                           {'data': {
                                               'items': [{'deliveryDate': {'endDate': endDate}}],
